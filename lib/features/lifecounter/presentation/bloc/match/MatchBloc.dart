...
class MatchBloc extends Bloc<MatchEvent, MatchState>{
  ...
  MatchBloc({
    ...
    _matchStatusObserver = matchController.status.listen((MatchStatus status) {
      if (status == MatchStatus.updated || status == MatchStatus.inProgress)
        add(MatchAvailable());
    });
    _gameStatusObserver = gameRepository.status.listen((GameStatus gameAvailable) {
      if(gameAvailable == GameStatus.inProgress)
        add(GameUpdated());
      }
    );
  }
  ...
  Future<MatchState> _mapMatchAvailableToState(MatchAvailable event, MatchState state) async {
    return await matchController.getCurrentMatch()
        .then((Match match) => state.copyWith(
          match: match,
  ...
  Future<MatchState> _mapGameAvailableToState(GameUpdated event, MatchState state) async {
    return await gameRepository.getCurrentGame()
        .then((Game recoveredGame) => state.copyWith(
          game: recoveredGame
    ));
  }
