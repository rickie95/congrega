...
Tournament tournament = new Tournament(
    ...
    playerList: {},
    adminList: {User(id: "a273be4d-9597-48a7-a10d-c4a29fb6750b", username: "morbidezzaRick")},
    judgeList: {},
    type: "Limited",
...
void main() {
  group('TournamentHttpClient', () {
    test('should retrieves a list of events.', () async {
      ...
      TournamentHttpClient tournamentClient =
          new TournamentHttpClient(restClient: http.Client(), graphQLClient: graphQLClient);
    ...
    test("should retrive a single event by its ID", () async {
      ...
      Tournament returnedEvent = await tournamentClient.getEventByUUID(tournament.id);
    ...
    test("should successfully send a new event to the server", () async {
      ...
      TournamentHttpClient tournamentClient =
          new TournamentHttpClient(restClient: client, graphQLClient: ArcanoGraphQLClient());

      Tournament returnedEvent = await tournamentClient.sendNewEvent(tournament);

      expect(returnedEvent.name, equals(tournament.name));
    });
  });
}
